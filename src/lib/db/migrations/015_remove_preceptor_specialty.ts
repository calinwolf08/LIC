...
/**
 * Migration 015: Remove Preceptor Specialty
 *
 * Removes the specialty column from preceptors table.
 */
...
export async function up(db: Kysely<any>): Promise<void> {
	...
	// Create new table without specialty
	await db.schema
	...
	// Copy existing data (excluding specialty)
...
export async function down(db: Kysely<any>): Promise<void> {
	...
	// Create new table with specialty
	await db.schema
		.createTable('preceptors_new')
		.addColumn('id', 'text', (col) => col.primaryKey())
		.addColumn('name', 'text', (col) => col.notNull())
		.addColumn('email', 'text', (col) => col.notNull().unique())
		.addColumn('specialty', 'text', (col) => col.notNull().defaultTo(''))
		.addColumn('max_students', 'integer', (col) => col.notNull().defaultTo(1))
		.addColumn('health_system_id', 'text', (col) =>
			col.references('health_systems.id').onDelete('set null')
		)
