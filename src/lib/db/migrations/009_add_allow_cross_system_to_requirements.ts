...
/**
 * Add allow_cross_system to Requirements Migration
 *
 * Adds allow_cross_system field to clerkship_requirements table.
 * This controls whether an elective requirement can be fulfilled
 * across different health systems or must stay within one system.
 */
...
export async function up(db: Kysely<any>): Promise<void> {
	...
	// Create new table with allow_cross_system field
	await db.schema
	...
	// Copy existing data, setting allow_cross_system to 0 (false) by default
...
export async function down(db: Kysely<any>): Promise<void> {
	...
	// Create table without allow_cross_system field
	await db.schema
		.createTable('clerkship_requirements_new')
		.addColumn('id', 'text', (col) => col.primaryKey())
		.addColumn('clerkship_id', 'text', (col) =>
	...
	// Copy data back (dropping allow_cross_system field)
	for (const requirement of existingRequirements) {
		const { allow_cross_system, ...rest } = requirement;
