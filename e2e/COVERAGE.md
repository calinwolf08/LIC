# E2E coverage map

<!-- GENERATED by `npm run coverage:map` (e2e/scripts/generate-coverage.ts). Do not edit by hand. -->

Traceability from product requirements (`R*`/`G*`) and review findings/decisions
(`P-*`, `F-*`, `D*`) to the journeys that exercise them. Derived from the specs:
journey ids from `describe` titles, tags from their `tag:` arrays, and requirements/
findings from `// @coverage @req(...)` / `@finding(...)` annotations.

## Summary

- Spec files: **61** across 9 phases
- Journeys: **46**
- Requirements mapped: **50**; findings/decisions mapped: **43**
- Tags: `@long` ×6, `@smoke` ×2, `@stage1` ×45, `@stage2` ×17, `@tenant` ×1

## Journeys by phase

| Phase | Journey(s) | Spec | Tags | Requirements | Findings/decisions |
| ----- | ---------- | ---- | ---- | ------------ | ------------------ |
| 0 | — | `phase-0/fixtures.smoke.spec.ts` | @smoke @stage1 | — | — |
| 1 | J1.1 | `phase-1/account-lifecycle.spec.ts` | @stage1 | R1.1, R1.2 | — |
| 1 | J1.5 | `phase-1/guard-and-errors.spec.ts` | @stage1 | R10.x | P1-e |
| 1 | J1.2 | `phase-1/schedule-lifecycle.spec.ts` | @stage1 | R1.3 | P1-c |
| 1 | J1.3 | `phase-1/schedule-scoping.spec.ts` | @stage1 @tenant | R1.3 | F-02, F-03, F-04, F-27 |
| 1 | J1.4 | `phase-1/shell-and-gating.spec.ts` | @stage1 | G1, G7, R1.1, R1.2, R10.x | — |
| 2 | J2.7 | `phase-2/availability-editing.spec.ts` | @stage1 | R3.3, R3.4 | P8-d |
| 2 | J2.4 | `phase-2/clerkship-config.spec.ts` | @stage1 | R4.1, R4.2, R4.3, R4.4 | — |
| 2 | J2.5, J2.6 | `phase-2/drill-through-and-lists.spec.ts` | @stage1 | R2.1, R2.2, R2.3, R10.x | — |
| 2 | J2.1 | `phase-2/locations.spec.ts` | @stage1 | R5.1, R5.2 | — |
| 2 | J2.2 | `phase-2/preceptor-wizard.spec.ts` | @stage1 | R3.1, R3.2, R3.3, R3.4, R3.5, R3.6 | — |
| 2 | J2.3 | `phase-2/student-hub.spec.ts` | @stage1 | R2.1, R2.2, R2.3 | — |
| 3 | J3.9 | `phase-3/availability-ripple.spec.ts` | @stage1 | R3.4, R3.5 | — |
| 3 | J3.10 | `phase-3/capacity-side-effects.spec.ts` | @stage1 | R7.1, R7.2, R7.3 | P8-c |
| 3 | J3.5 | `phase-3/edit-lifecycle.spec.ts` | @stage1 | R6.5, R6.6 | G8 |
| 3 | J3.7 | `phase-3/electives.spec.ts` | @stage1 | R2.1, R4.2, R4.3 | — |
| 3 | — | `phase-3/entity-tab-deeplink.spec.ts` | @stage1 | R10.x | — |
| 3 | J3.1 | `phase-3/entry-points.spec.ts` | @stage1 | R3.1, R6.1, R6.2, R6.3, R6.4, R6.5, R6.6, R6.7 | — |
| 3 | J3.4 | `phase-3/hard-blocks.spec.ts` | @stage1 | R7.1, R7.2 | — |
| 3 | J3.8 | `phase-3/locks.spec.ts` | @stage2 | R6.7 | — |
| 3 | J3.2 | `phase-3/range-creation.spec.ts` | @stage1 | R6.1, R6.2 | — |
| 3 | J3.3 | `phase-3/soft-codes.spec.ts` | @stage1 | R7.1, R7.2, R7.3, R7.4 | — |
| 3 | J3.6 | `phase-3/swap.spec.ts` | @stage1 | R6.6 | — |
| 4 | J4.2 | `phase-4/blackout-dates.spec.ts` | @stage1 | R8.4 | P4-d, P4-e |
| 4 | J4.1 | `phase-4/calendar-workspace.spec.ts` | @stage1 | R8.1, R8.2, R8.3 | P4-a, P4-b |
| 4 | J4.5 | `phase-4/dashboard.spec.ts` | @stage1 | R3.4, R9.1, R9.2 | — |
| 4 | J4.3 | `phase-4/export.spec.ts` | @stage1 | R8.5 | P4-f |
| 4 | J4.4 | `phase-4/schedule-health.spec.ts` | @stage1 | R7.1, R7.2, R7.3, R7.4 | — |
| 5 | J5.3 | `phase-5/bypass.spec.ts` | @stage2 | G9 | D5-1, P5-b, P5-c |
| 5 | J5.5 | `phase-5/config.spec.ts` | @stage2 | G5, R4.1 | — |
| 5 | J5.4 | `phase-5/fallbacks.spec.ts` | @stage2 | G10 | P5-d |
| 5 | J5.1 | `phase-5/first-run.spec.ts` | @stage2 | G2, G3, G4, R3.1 | P5-a |
| 5 | J5.2 | `phase-5/regenerate.spec.ts` | @stage2 | G2, G4 | — |
| 5 | J5.7 | `phase-5/results.spec.ts` | @stage2 | G4 | — |
| 5 | J5.6 | `phase-5/teams.spec.ts` | @stage2 | G6, G11 | — |
| 6 | J6.2 | `phase-6/gating.spec.ts` | @stage1 | G1, G7 | P6-a |
| 6 | J6.1 | `phase-6/generated-rows-ordinary.spec.ts` | @stage2 | G8, G12 | — |
| 6 | J6.3 | `phase-6/parity-without-teams.spec.ts` | @stage2 | G11 | — |
| 6 | J6.5 | `phase-6/tenant-isolation.spec.ts` | @stage2 | — | F-02, F-03, F-04, F-27 |
| 6 | J6.4 | `phase-6/validation-parity.spec.ts` | @stage1 | G8, R7.1 | — |
| 7 | J7.7 | `phase-7/config-edits.spec.ts` | @stage1 | R1.3, R4.2, R4.3, R5.1 | P8-e |
| 7 | J7.4 | `phase-7/dependency-chain.spec.ts` | @long @stage1 | R5.1, R5.2 | D7-2 |
| 7 | J7.1 | `phase-7/semester.spec.ts` | @long @stage2 | R1.3, R4.2 | D7-1, P7-a |
| 7 | J7.2 | `phase-7/shared-entities.spec.ts` | @long @stage1 | R1.3 | D7-3, D7-4 |
| 7 | J7.1 | `phase-7/smoke.spec.ts` | @smoke @stage2 | — | — |
| 7 | J7.3 | `phase-7/time-boundaries.spec.ts` | @long @stage2 | — | F-22 |
| 7 | J7.5 | `phase-7/two-tabs.spec.ts` | @long @stage2 | R6.4 | — |
| 7 | J7.6 | `phase-7/volume.spec.ts` | @long @stage2 | — | F-30 |
| 9 | — | `phase-9/assign-outside-availability.spec.ts` | @stage1 | R3.6, R7.4 | CF-I1 |
| 9 | — | `phase-9/availability-empty-pattern.spec.ts` | @stage1 | R3.2, R3.3 | CF-H1 |
| 9 | — | `phase-9/availability-preference-consumption.spec.ts` | @stage1 @stage2 | R3.2 | CF-H8 |
| 9 | — | `phase-9/availability-preference.spec.ts` | @stage1 | R3.2 | CF-H8 |
| 9 | — | `phase-9/core-preceptor.spec.ts` | @stage1 | R7.4 | CF-F5 |
| 9 | — | `phase-9/page-context-and-metric-labels.spec.ts` | @stage1 | R10.1 | CF-N1 |
| 9 | — | `phase-9/preceptor-phone-type.spec.ts` | @stage1 | — | CF-F1 |
| 9 | — | `phase-9/schedule-end-before-start.spec.ts` | @stage1 | R1.3 | CF-C1 |
| 9 | — | `phase-9/signup-invalid-email.spec.ts` | @stage1 | — | CF-A1 |
| 9 | — | `phase-9/team-serveable.spec.ts` | @stage1 | — | CF-G3, CF-G4 |
| 9 | — | `phase-9/team-single-member.spec.ts` | @stage1 | — | CF-G1 |
| 9 | — | `phase-9/wizard-activates-schedule.spec.ts` | @stage1 | R1.2, R1.3 | CF-B1 |
| 9 | — | `phase-9/wizard-clarity.spec.ts` | @stage1 | R1.3 | CF-C4, CF-C5, CF-D2 |

## Requirement → journeys

| Requirement | Journeys |
| ----------- | -------- |
| G1 | J1.4, J6.2 |
| G2 | J5.1, J5.2 |
| G3 | J5.1 |
| G4 | J5.1, J5.2, J5.7 |
| G5 | J5.5 |
| G6 | J5.6 |
| G7 | J1.4, J6.2 |
| G8 | J6.1, J6.4 |
| G9 | J5.3 |
| G10 | J5.4 |
| G11 | J5.6, J6.3 |
| G12 | J6.1 |
| R1.1 | J1.1, J1.4 |
| R1.2 | J1.1, J1.4, wizard-activates-schedule.spec.ts |
| R1.3 | J1.2, J1.3, J7.1, J7.2, J7.7, schedule-end-before-start.spec.ts, wizard-activates-schedule.spec.ts, wizard-clarity.spec.ts |
| R2.1 | J2.3, J2.5, J2.6, J3.7 |
| R2.2 | J2.3, J2.5, J2.6 |
| R2.3 | J2.3, J2.5, J2.6 |
| R3.1 | J2.2, J3.1, J5.1 |
| R3.2 | availability-empty-pattern.spec.ts, availability-preference-consumption.spec.ts, availability-preference.spec.ts, J2.2 |
| R3.3 | availability-empty-pattern.spec.ts, J2.2, J2.7 |
| R3.4 | J2.2, J2.7, J3.9, J4.5 |
| R3.5 | J2.2, J3.9 |
| R3.6 | assign-outside-availability.spec.ts, J2.2 |
| R4.1 | J2.4, J5.5 |
| R4.2 | J2.4, J3.7, J7.1, J7.7 |
| R4.3 | J2.4, J3.7, J7.7 |
| R4.4 | J2.4 |
| R5.1 | J2.1, J7.4, J7.7 |
| R5.2 | J2.1, J7.4 |
| R6.1 | J3.1, J3.2 |
| R6.2 | J3.1, J3.2 |
| R6.3 | J3.1 |
| R6.4 | J3.1, J7.5 |
| R6.5 | J3.1, J3.5 |
| R6.6 | J3.1, J3.5, J3.6 |
| R6.7 | J3.1, J3.8 |
| R7.1 | J3.3, J3.4, J3.10, J4.4, J6.4 |
| R7.2 | J3.3, J3.4, J3.10, J4.4 |
| R7.3 | J3.3, J3.10, J4.4 |
| R7.4 | assign-outside-availability.spec.ts, core-preceptor.spec.ts, J3.3, J4.4 |
| R8.1 | J4.1 |
| R8.2 | J4.1 |
| R8.3 | J4.1 |
| R8.4 | J4.2 |
| R8.5 | J4.3 |
| R9.1 | J4.5 |
| R9.2 | J4.5 |
| R10.1 | page-context-and-metric-labels.spec.ts |
| R10.x | entity-tab-deeplink.spec.ts, J1.4, J1.5, J2.5, J2.6 |

## Finding / decision → journeys

| Finding / decision | Journeys |
| ------------------ | -------- |
| CF-A1 | signup-invalid-email.spec.ts |
| CF-B1 | wizard-activates-schedule.spec.ts |
| CF-C1 | schedule-end-before-start.spec.ts |
| CF-C4 | wizard-clarity.spec.ts |
| CF-C5 | wizard-clarity.spec.ts |
| CF-D2 | wizard-clarity.spec.ts |
| CF-F1 | preceptor-phone-type.spec.ts |
| CF-F5 | core-preceptor.spec.ts |
| CF-G1 | team-single-member.spec.ts |
| CF-G3 | team-serveable.spec.ts |
| CF-G4 | team-serveable.spec.ts |
| CF-H1 | availability-empty-pattern.spec.ts |
| CF-H8 | availability-preference-consumption.spec.ts, availability-preference.spec.ts |
| CF-I1 | assign-outside-availability.spec.ts |
| CF-N1 | page-context-and-metric-labels.spec.ts |
| D5-1 | J5.3 |
| D7-1 | J7.1 |
| D7-2 | J7.4 |
| D7-3 | J7.2 |
| D7-4 | J7.2 |
| F-02 | J1.3, J6.5 |
| F-03 | J1.3, J6.5 |
| F-04 | J1.3, J6.5 |
| F-22 | J7.3 |
| F-27 | J1.3, J6.5 |
| F-30 | J7.6 |
| G8 | J3.5 |
| P1-c | J1.2 |
| P1-e | J1.5 |
| P4-a | J4.1 |
| P4-b | J4.1 |
| P4-d | J4.2 |
| P4-e | J4.2 |
| P4-f | J4.3 |
| P5-a | J5.1 |
| P5-b | J5.3 |
| P5-c | J5.3 |
| P5-d | J5.4 |
| P6-a | J6.2 |
| P7-a | J7.1 |
| P8-c | J3.10 |
| P8-d | J2.7 |
| P8-e | J7.7 |

## Guard

No `test.only` / `test.skip` / `test.fixme` in `e2e/journeys` ✅

