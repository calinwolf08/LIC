 ...
 *
 * FINDING P1-e (spec R10.3 — unsaved-changes guard): no form in the app guards
 * navigation away from unsaved edits. `FormShell` implements the guard but is
 * imported by zero forms; the availability builder shows an "Unsaved changes"
 * indicator but does not block navigation. The last test documents the current
 * (unguarded) behaviour so it flips the day a guard is wired in.
 */
...
test.describe('J1.5 error surfacing & unsaved changes', { tag: ['@stage1'] }, () => {
	...
	test('FINDING P1-e: navigating away from an unsaved form is NOT guarded today', async ({
		asAdmin,
		sandbox
	}) => {
		// Documents current behaviour (R10.3 unimplemented): typing into a
		// multi-field form and navigating away raises no "discard changes?" dialog.
		// When a guard is wired in (FormShell), flip this to assert the dialog.
		...
		// No guard today: navigation proceeds straight to the dashboard.
